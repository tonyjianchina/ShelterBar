...
从 [GitHub Releases v0.3.1](https://github.com/tonyjianchina/ShelterBar/releases/tag/v0.3.1) 下载 [ShelterBar-0.3.1-macos-arm64.dmg](https://github.com/tonyjianchina/ShelterBar/releases/download/v0.3.1/ShelterBar-0.3.1-macos-arm64.dmg)，打开后将 **ShelterBar.app** 拖入 **Applications**，随后推出磁盘映像。也可下载 [ZIP](https://github.com/tonyjianchina/ShelterBar/releases/download/v0.3.1/ShelterBar-0.3.1-macos-arm64.zip)，解压后将应用移到“应用程序”文件夹。
...
## v0.3.1 原生拖拽、原始状态图标与新版应用图标

本版沿用 v0.3.0 的原生拖拽实现，并改用更清晰、更轻量的应用图标。应用匹配真实的菜单栏状态窗口，拖拽时分别路由按下/移动事件和释放事件，并对图标透明留白、刘海区可见范围、分隔符完整宽度以及多显示器边界进行安全检查。由 Control Center 托管的第三方状态窗口只在进程、图层和几何范围唯一匹配时使用。收纳或恢复必须通过移动后的实际几何位置验证，否则不会更改保存状态。
...
将两个安装包及 [SHA256SUMS](https://github.com/tonyjianchina/ShelterBar/releases/download/v0.3.1/SHA256SUMS) 下载到同一个文件夹，在该文件夹运行：
...
```sh
shasum -a 256 ShelterBar-0.3.1-macos-arm64.dmg
```
...
```text
dist/ShelterBar.app
dist/releases/v0.3.1/ShelterBar-0.3.1-macos-arm64.dmg
dist/releases/v0.3.1/ShelterBar-0.3.1-macos-arm64.zip
dist/releases/v0.3.1/SHA256SUMS
```
...
- 将 `v0.3.1` 标记为 **Pre-release**，公开说明系统要求、两项权限、未公证状态和已知限制。
